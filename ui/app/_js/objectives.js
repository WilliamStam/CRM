...
function getData() {
	...
	$.getData("/app/data/objectives/data", uri, function (data) {
		...
		if (data.options.filter.daterange){
			var parts,start,end;
			if (data.options.filter.daterange.indexOf(" to ")>0){
				parts = data.options.filter.daterange.split(" to ");
				start = moment(parts[0],"YYYY-MM-DD");
				end = moment(parts[1],"YYYY-MM-DD");
			} else {
				parts = ranges[data.options.filter.daterange];
				start = parts[0];
				end = parts[1];
			}
		...
		cb(false, false, false, data.options.filter.daterange);
