...
class objectives extends _ {
	...
	function data() {
		$return = array();
		$groupby = models\objectives::getInstance()->_groupby();
		$columns = models\objectives::getInstance()->_columns();

		$default_settings = models\system_users::default_settings("objectives");

		$settings = $this->_settings("objectives", array("groupby","groupby_dir","order","order_dir","order_dir","columns","search","mine"),array("groupby"=>$groupby,"columns"=>$columns));
		...
		if ($settings['search']){
			$search_str = models\search::getInstance()->tasks($settings['search']);
