...
class tasks extends _ {
	...
	function data() {
		$return = array();
		$groupby = models\tasks::getInstance()->_groupby();
		$columns = models\tasks::getInstance()->_columns();

		$default_settings = models\system_users::default_settings("tasks");

		$settings = $this->_settings("tasks", array("groupby","groupby_dir","order","order_dir","order_dir","columns","search","mine"),array("groupby"=>$groupby,"columns"=>$columns));
