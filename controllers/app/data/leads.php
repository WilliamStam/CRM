...
class leads extends _ {
	...
	function data() {
		$return = array();
		$groupby = models\leads::getInstance()->_groupby();
		$columns = models\leads::getInstance()->_columns();

		$default_settings = models\system_users::default_settings("leads");

		$settings = $this->_settings("leads", array("groupby","groupby_dir","order","order_dir","order_dir","columns","search","mine"),array("groupby"=>$groupby,"columns"=>$columns));
		...
		if ($settings['search']){
			$search_str = models\search::getInstance()->leads($settings['search']);
		...
		*/
		...
		$records = models\leads::getInstance()->getAll($where_sql,$orderby,"",$options);
